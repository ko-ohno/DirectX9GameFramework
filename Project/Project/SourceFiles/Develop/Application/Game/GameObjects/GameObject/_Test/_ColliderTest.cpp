...
void ColliderTest::UpdateGameObject(float deltaTime)
{
	...
	//OBBの衝突判定の作成
	{
		...
		// 衝突判定を更新
		obb_collider_component_->SetOffsetTranslation(box_pos);
		...
		// 衝突判定Aを更新
		obb_collider_component_a_->SetOffsetTranslation(box_end_pos);
	...
	//スフィアの衝突判定の作成
	{
		...
		// 衝突判定を更新
		sphere_collider_component_->SetOffsetTranslation(sphere_pos);
		...
		// 衝突判定Aを更新
		sphere_collider_component_a_->SetOffsetTranslation(sphere_end_pos);
