...
void MessageLoop::Run(void)
{
	...
	for (;;)
	{
		...
		if ((msg.message == WM_QUIT) || i_manager_->IsLoopBreak() || i_manager_->IsShutDown()) { break; }
	}
...
/* 画面の拡縮倍率の取得
-----------------------------------------------------------------------------*/
float MessageLoop::GetScreenScaler(void)
{
	if (i_manager_ == nullptr) { return 1.0f; }
	return i_manager_->GetScreenScaler();
}
