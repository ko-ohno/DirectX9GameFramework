...
namespace ImGui
{
	...
	//（開始=スタックにウィンドウをプッシュして追加を開始します。終了=スタックからポップウィンドウを開きます。同じフレーム内で同じウィンドウに複数回追加することができます。）
	// Begin（）/ BeginChild（）は、ウィンドウが折りたたまれているか完全に切り取られていることを示すためにfalseを返すので、早めにウィンドウに何も送信しないでください。
	//戻り値に関係なく、必ずBegin（）/ BeginChild（）の呼び出しに対応するEnd（）/ EndChild（）を呼び出す必要があります（これは従来の理由によるもので、BeginMenu / EndMenu、BeginPopup / EndPopupと矛盾します）。対応するBegin関数がtrueを返した場合にのみEnd呼び出しを呼び出す必要があるその他の関数
	// 'bool * p_open！= NULL'を渡すと、ウィンドウの右上隅に閉じるウィジェットが表示されます。これをクリックすると、ブール値がfalseに設定されます。
